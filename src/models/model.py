...
class IRNet(BasicModel):
    ...
    def __init__(self, args, grammar):
        ...
        input_dim = args.action_embed_size + args.att_vec_size + args.type_embed_size
        # previous action
        # input feeding
        # pre type embedding
        ...
        self.col_type = nn.Linear(2, args.col_embed_size)
    ...
    def forward(self, examples):
        ...
        table_embedding = self.gen_x_batch(batch.table_sents)  # 所有列名列表
        src_embedding = self.gen_x_batch(batch.src_sents)
        schema_embedding = self.gen_x_batch(batch.table_names)  # 表名列表
