...
class AuxiliaryPointerNet(nn.Module):
    ...
    def forward(self, src_encodings, src_context_encodings, src_token_mask, query_vec):
        ...
        encodings = src_encodings.clone()  # (batch_size, src_sent_len, src_encoding_size)
        context_encodings = src_context_encodings.clone()  # (batch_size, src_sent_len, src_encoding_size)
        if self.attention_type == 'affine':
            encodings = self.src_encoding_linear(src_encodings)  # (batch_size, src_sent_len, query_vec_size)
            context_encodings = self.auxiliary_encoding_linear(src_context_encodings)  # (batch_size, src_sent_len, query_vec_size)
        encodings = encodings.unsqueeze(1)  # (batch_size, 1, src_sent_len, query_vec_size)
        context_encodings = context_encodings.unsqueeze(1)  # (batch_size, 1, src_sent_len, query_vec_size)
