...
from src.models.tokens_from_transformers import transformers_tokens
...
class BasicModel(nn.Module):
        ...
        for i, one_q in enumerate(q):
            if not is_list:
                ...
                for ws in one_q:
                    ...
                    ws_len = len(ws)
                    for w in ws:
                        emb_list.append(self.word_emb.get(w, self.word_emb['无']))
                    if ws_len == 0:
    ...
    #         is_list = True
    #     for i, one_q in enumerate(q):
    #         q_val = transformers_tokens(one_q, is_list)
    #
    #         val_embs.append(q_val)
    #         val_len[i] = len(q_val)
    #     max_len = max(val_len)
