...
def dot_prod_attention(h_t, src_encoding, src_encoding_att_linear, mask=None):
    ...
    att_view = (att_weight.size(0), 1, att_weight.size(1))  # (batch_size, 1, src_sent_len)
    # (batch_size, hidden_size)
    ctx_vec = torch.bmm(att_weight.view(*att_view), src_encoding).squeeze(1)  # (batch_size, hidden_size * 2)
