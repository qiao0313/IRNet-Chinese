...
class Example:
    ...
    def __init__(self, src_sent, tgt_actions=None, vis_seq=None, tab_cols=None, col_num=None, sql=None,
        ...
        self.src_sent = src_sent  # 句子n-gram匹配列表
        self.tokenized_src_sent = tokenized_src_sent
        self.vis_seq = vis_seq  # (question, 列名集合, SQL)
        self.tab_cols = tab_cols  # col_set列名分割成字符列表
        self.col_num = col_num  # 列的个数
        self.sql = sql  # SQL
        self.one_hot_type = one_hot_type  # question的3种匹配类型
        self.col_hot_type = col_hot_type  # col_set的4种匹配类型
        self.schema_len = schema_len
        self.tab_ids = tab_ids
        self.table_names = table_names  # 表名列表
        self.table_len = table_len  # 表的个数
        self.col_table_dict = col_table_dict  # 每个列所属于的table_id
        self.cols = cols  # 每个表包含的列集合
        self.table_col_name = table_col_name  # 每个table包含的列名字
        self.table_col_len = table_col_len  # 表的个数
        self.col_pred = col_pred
        self.tgt_actions = tgt_actions  # IR序列
        self.truth_actions = copy.deepcopy(tgt_actions)

        self.sketch = list()  # 去掉C, T, A之后的IR序列
...
class Batch(object):
    def __init__(self, examples, grammar, cuda=False):
        ...
        self.table_sents_word = [["".join(x) for x in e.tab_cols] for e in self.examples]  # col_set列表

        self.schema_sents_word = [["".join(x) for x in e.table_names] for e in self.examples]  # 表名列表

        self.src_type = [e.one_hot_type for e in self.examples]  # question的3种匹配类型
        self.col_hot_type = [e.col_hot_type for e in self.examples]  # col_set的4种匹配类型
        self.table_sents = [e.tab_cols for e in self.examples]  # col_set列名分割成字符列表
        self.col_num = [e.col_num for e in self.examples]
        self.tab_ids = [e.tab_ids for e in self.examples]
        self.table_names = [e.table_names for e in self.examples]  # 所有表名分割成字符列表
        self.table_len = [e.table_len for e in examples]
        self.col_table_dict = [e.col_table_dict for e in examples]  # 每个列所属于的table_id
        self.table_col_name = [e.table_col_name for e in examples]  # 每个table包含的列名字
        self.table_col_len = [e.table_col_len for e in examples]  # 表的个数
