...
def load_dataSets(args):
    with open(args.input_path, 'r', encoding='utf8') as f:
        datas = json.load(f)
    with open(os.path.join(args.data_path, 'db_schema.json'), 'r', encoding='utf8') as f:
...
def is_partial_match(query, table_names):
    table_names = [[x for x in names] for names in table_names]
    same_count = 0
    result = None
...
def find_table(cur_table, origin_table_names, question_arg_type, question_arg):
    h_table = None
    for i in range(len(question_arg_type))[::-1]:  # reverse
        if question_arg_type[i] == ['表']:
...
def alter_not_in(datas, schemas):
    for d in datas:
        if 'Filter(19)' in d['model_result']:
            ...
            current_table['schema_content_clean'] = [x[1] for x in current_table['column_names']]  # 所有表的列名集合
            current_table['col_table'] = [col[0] for col in current_table['column_names']]  # 列对应的表id
            origin_table_names = [[x for x in names] for names in d['table_names']]  # 表名分割成字符列表
            question_arg_type = d['question_arg_type']
            question_arg = d['question_arg']
            pred_label = d['model_result'].split(' ')  # label列表
...
def alter_inter(datas):
    for d in datas:
        if 'Filter(0)' in d['model_result']:
            now_result = d['model_result'].split(' ')  # label列表
...
def alter_column0(datas):
    ...
    for d in datas:
        if 'C(0)' in d['model_result']:
            ...
            question_arg_type = d['question_arg_type']
            question_arg = d['question_arg']
            table_names = [[name for name in names] for names in d['table_names']]
            origin_table_names = [[x for x in names] for names in d['table_names']]
            count += 1
            easy_flag = False
            for q_ind, q in enumerate(d['question_arg']):
                # q = [x for x in q]
                q_str = "".join("".join(x) for x in d['question_arg'])
                if '有多少' in q_str or '几' in q_str or '数量' in q_str:  # 根据具体数据集修改
                    easy_flag = True
            if easy_flag:
                # check for the last one is a table word
                for q_ind, q in enumerate(d['question_arg']):
                    if (q_ind > 0 and q == ['少'] and d['question_arg'][q_ind - 1] == ['多']) or (
                                q_ind > 0 and q == ['量'] and d['question_arg'][q_ind - 1] == ['数']) or (
                                q_ind > 0 and q == ['几']):
                        ...
                        if re is not False:
                            ...
                            result.append((d['query'], d['question'], table_result, d))  # table_result: table_name分割成字符列表
                            break
                        else:
