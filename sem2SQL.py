...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    levels_correct = {'easy': 0, 'medium': 0, 'hard': 0, 'extra': 0}
    levels_num = {'easy': 0, 'medium': 0, 'hard': 0, 'extra': 0}
    ...
    with open(args.output_path, 'w', encoding='utf8') as d, open('gold.txt', 'w', encoding='utf8') as g:
        for i in index:
            try:
                hardness = evaluator.eval_hardness(datas[i]["sql"])
                levels_num[hardness] += 1
                if datas[i]['model_result'] == datas[i]['rule_label']:
                    levels_correct[hardness] += 1
    ...
    easy_correct = levels_correct['easy'] / levels_num['easy']
    medium_correct = levels_correct['medium'] / levels_num['medium']
    hard_correct = levels_correct['hard'] / levels_num['hard']
    extra_correct = levels_correct['extra'] / levels_num['extra']
    print(levels_num['easy'], levels_num['medium'], levels_num['hard'], levels_correct['extra'])
    print('easy:%.4f\tmedium:%.4f\thard:%.4f\textra:%.4f' % (easy_correct, medium_correct, hard_correct, extra_correct))
